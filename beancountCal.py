...
class Importer(importer.ImporterProtocol):
    "An importer for Visa CAL PDF files"
    ...
    def extract(self, f):
        entries = []
        path = f.name
        raw = cal.getPdfData(path)
        table = cal.fixData(raw)
        for index, row in enumerate(table):
            ...
            txn_payee = row[1]
            txn_amount = D(row[5].rstrip('0').rstrip('.'))
            txn_payment = D(row[6].rstrip('0').rstrip('.'))
